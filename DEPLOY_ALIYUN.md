...
- 新企业可以在首页自助注册，默认获得 1 个月、不限制成员人数的免费试用。
- 首次 seed 会给 demo 企业开通 `BUSINESS` 套餐和 30 个席位。
- 正式客户可由企业管理员按启用成员数创建订阅订单；线下转账由平台超级管理员确认，微信支付可通过 Native 扫码和回调自动开通套餐。
- 生产微信支付需要配置商户号、商户证书序列号、商户私钥、API v3 密钥、微信支付平台公钥和平台公钥序列号。
