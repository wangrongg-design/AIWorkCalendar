...
const env = [
  ...
  "",
  `EXPORT_STORAGE_DIR=${value(config.exports?.containerDir ?? "/app/storage/exports")}`,
  `EXPORT_STORAGE_HOST_DIR=${value(config.exports?.hostDir ?? "./storage/exports")}`,
  "",
  `WORK_LOG_ATTACHMENT_DIR=${value(config.workLogAttachments?.containerDir ?? "/app/storage/work-log-attachments")}`,
  `WORK_LOG_ATTACHMENT_HOST_DIR=${value(config.workLogAttachments?.hostDir ?? "./storage/work-log-attachments")}`,
  "",
  `BILLING_PAYMENT_MODE=${value(config.billing?.paymentMode ?? "mock")}`,
  `PUBLIC_WEB_URL=${value(config.billing?.publicWebUrl ?? config.domains?.webUrl)}`,
  `PUBLIC_API_URL=${value(config.billing?.publicApiUrl ?? config.domains?.apiUrl)}`,
  `ALIPAY_APP_ID=${value(config.billing?.alipay?.appId)}`,
  `ALIPAY_PRIVATE_KEY=${value(config.billing?.alipay?.privateKey)}`,
  `ALIPAY_NOTIFY_URL=${value(config.billing?.alipay?.notifyUrl)}`,
  `ALIPAY_RETURN_URL=${value(config.billing?.alipay?.returnUrl)}`,
  `WECHAT_PAY_APP_ID=${value(config.billing?.wechatPay?.appId)}`,
  `WECHAT_PAY_MCH_ID=${value(config.billing?.wechatPay?.merchantId)}`,
  `WECHAT_PAY_MCH_SERIAL_NO=${value(config.billing?.wechatPay?.merchantSerialNo)}`,
  `WECHAT_PAY_PRIVATE_KEY=${value(config.billing?.wechatPay?.privateKey)}`,
  `WECHAT_PAY_PRIVATE_KEY_PATH=${value(config.billing?.wechatPay?.privateKeyPath)}`,
  `WECHAT_PAY_API_V3_KEY=${value(config.billing?.wechatPay?.apiV3Key)}`,
  `WECHAT_PAY_PLATFORM_SERIAL_NO=${value(config.billing?.wechatPay?.platformSerialNo)}`,
  `WECHAT_PAY_PLATFORM_PUBLIC_KEY=${value(config.billing?.wechatPay?.platformPublicKey)}`,
  `WECHAT_PAY_PLATFORM_PUBLIC_KEY_PATH=${value(config.billing?.wechatPay?.platformPublicKeyPath)}`,
  `WECHAT_PAY_NOTIFY_URL=${value(config.billing?.wechatPay?.notifyUrl)}`,
  `WECHAT_PAY_RETURN_URL=${value(config.billing?.wechatPay?.returnUrl)}`,
  "",
