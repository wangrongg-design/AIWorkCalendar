...
- 小程序后台添加并启用“微信同声传译”插件
- 首次使用语音填报时，用户需要授权录音权限

本地如果微信开发者工具提示“模拟器启动失败”，先不要启用插件，直接使用当前默认 `app.json`。语音识别要等真实 AppID 在微信公众平台添加同声传译插件后，再把下面配置加回 `app.json`：
...
- 正式发布前需要替换真实 AppID，并完成微信平台插件授权、域名配置和隐私协议配置。
