...
export class OpenAiService {
  ...
  private localSuggestionSplitCount(text: string) {
    ...
    const softClauses = this.splitDraftClauses(text, true);
    if (softClauses.length >= 2 && this.shouldSplitDraftSoftly(text)) return softClauses.length;
  ...
  private hasDraftClauseContent(text: string) {
    if (this.isDraftSubmitCommand(text)) return false;
    if (this.isNonWorkSocialClause(text)) return false;
    const cleaned = text
  ...
  private hasWorkSuggestionSignal(text: string) {
    return /(项目|客户|需求|方案|会议|沟通|交流|会谈|洽谈|讨论|对接|确认|推进|评审|合同|报价|工程|系统|平台|实施|交付|风险|阻塞)/u.test(text);
  }

  private isNonWorkSocialClause(text: string) {
    return /(吃饭|用餐|午餐|晚餐|聚餐|喝茶|咖啡|茶歇)/u.test(text) && !this.hasWorkSuggestionSignal(text);
  }

  private isDraftSubmitCommand(text: string) {
    const compact = text.replace(/\s+/g, "");
  ...
  private inferDraftProjectHint(text: string) {
    ...
    for (const pattern of patterns) {
      ...
      if (value) {
        const raw = suffix ? `${value}${suffix}` : value;
        ...
        if (candidate.length >= 2 && !/^(今天|昨天|明天|客户)$/u.test(candidate)) {
          return candidate;
        }
        if (!/^(今天|昨天|明天|客户)$/u.test(raw)) {
          return raw;
        }
      }
    }
